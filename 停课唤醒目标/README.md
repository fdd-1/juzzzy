...
每月运营要做的事：把 BI 报表里「历史停课未唤醒」学员筛出来 → 在六一工作台建标签和用户群 → 打企微标签 → 在北极星建外呼任务 → 配置标签数据同步，让客服坐席跟进。

整套流程被拆成 8 个原子步骤，可以一键跑全流程，也可以从任意中间步骤补跑。
...
| 8 | `sync-tag-data` | 六一工作台配置标签数据同步：把小账号用户群挂到豌豆数合表，每天同步 | `output/sync_tag_final_{今天}.png` |
...
python tingke_wakeup.py sync-tag-data --month 2026-06
```
...
### `output/sync_tag_final_{今天}.png`
步骤 8 配置完成截图。验证标签数据同步配置已挂到小账号用户群，状态=启用、频率=每天。脚本不产 json，配置正确性以截图为准。
