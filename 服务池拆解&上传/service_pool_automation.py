...
def _default_path_vars(skill_dir):
    """SKILL.md 约定的路径变量及其默认值。环境变量优先。"""
    desktop = os.environ.get("DESKTOP") or str(Path.home() / "Desktop")
    bi_tool_default = os.environ.get("BI_TOOL_DIR")
    if not bi_tool_default:
        # 兼容两种常见布局：扁平 / 嵌套同名子目录
        candidates = (
            sorted(glob.glob(os.path.join(desktop, "smartbi-data-cli-internal-*", "smartbi-data-cli-internal-*", "scripts")))
            + sorted(glob.glob(os.path.join(desktop, "smartbi-data-cli-internal-*", "scripts")))
        )
        bi_tool_default = candidates[-1] if candidates else os.path.join(desktop, "smartbi-data-cli-internal-*", "scripts")
    return {
        "SKILL_DIR": os.environ.get("SKILL_DIR", skill_dir),
        "DESKTOP": desktop,
        "POOL_DIR": os.environ.get("POOL_DIR", os.path.join(desktop, "每月池子")),
        "LIUYI_DIR": os.environ.get("LIUYI_DIR", os.path.join(desktop, "六一标签")),
        "BI_TOOL_DIR": bi_tool_default,
    }

_VAR_PATTERN = re.compile(r"\$\{(\w+)\}")

def expand_path(value, vars_map):
    """把 ${VAR} 占位符替换为实际路径；非字符串原样返回。支持 BI_TOOL_DIR 这种含通配的路径。"""
    if not isinstance(value, str):
        return value
    expanded = _VAR_PATTERN.sub(lambda m: vars_map.get(m.group(1), m.group(0)), value)
    if "*" in expanded or "?" in expanded:
        matches = sorted(glob.glob(expanded))
        if matches:
            return matches[-1]  # 取字典序最大，多版本时取最新
    return expanded
...
class FailFast(Exception):
    """命中 fail-fast 关键字时抛出，跳过后续重试。"""

def run_with_retry(cmd, env, logger, *, label, max_attempts=1, delay=0,
                   fail_fast_patterns=None):
    """
    跑 subprocess，失败按需重试。stdout/stderr 命中 fail_fast_patterns 时立即停。
    成功返回 CompletedProcess；最终失败 raise Exception。
    """
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if result.returncode == 0:
            if attempt > 1:
                logger.info(f"  {label} 第 {attempt} 次重试成功")
            return result

        combined = f"{result.stdout}\n{result.stderr}"
        last_err = combined.strip()
        for hint, pattern in fail_fast_patterns:
            if pattern in combined:
                logger.error(f"  {label} 命中 fail-fast：{hint}")
                raise FailFast(f"{label} 失败（{hint}）：{last_err[-500:]}")

        if attempt < max_attempts:
            logger.warning(f"  {label} 第 {attempt}/{max_attempts} 次失败，{delay}s 后重试")
            if delay > 0:
                time.sleep(delay)
        else:
            logger.error(f"  {label} 重试 {max_attempts} 次仍失败")

    raise Exception(f"{label} 失败：{last_err[-500:]}")
...
def load_config(config_path="config.json"):
    """加载配置文件，并展开 ${VAR} 占位符（SKILL_DIR/DESKTOP/POOL_DIR/LIUYI_DIR/BI_TOOL_DIR）。"""
    ...
    skill_dir = os.path.dirname(os.path.abspath(config_path))
    vars_map = _default_path_vars(skill_dir)

    # 顶层路径字段统一展开
    path_keys = ["liuzhuang_file", "bi_skill_path", "liuyi_create_path",
                 "liuyi_sync_path", "work_dir_base", "smartbi_browser_export_path"]
    for k in path_keys:
        if k in config:
            config[k] = expand_path(config[k], vars_map)

    # 缺失字段用默认值兜底
    config.setdefault("work_dir_base", skill_dir)
    config.setdefault("liuyi_create_path",
                      os.path.join(vars_map["LIUYI_DIR"], "create.py"))
    config.setdefault("liuyi_sync_path",
                      os.path.join(skill_dir, "sync_tag_data.py"))
    if not config.get("smartbi_browser_export_path"):
        bi_tool = expand_path(
            os.path.join("${BI_TOOL_DIR}", "smartbi_browser_export.py"), vars_map)
        config["smartbi_browser_export_path"] = bi_tool

    config["_skill_dir"] = skill_dir
    config["_vars"] = vars_map
    return config
...
def create_liuyi_tag_and_group(excel_path, month_str, config, logger, dry_run=False):
    ...
    result = run_with_retry(
        ...
        delay=5,
        fail_fast_patterns=[
            ("dadou_ids 列格式不对（code:101）", "code: 101"),
            ("auth_state.json 过期，请重跑 login.py", "找不到「新增」"),
            ("auth_state.json 过期，请重跑 login.py", "登录态失效"),
        ],
    )
