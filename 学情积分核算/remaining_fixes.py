"""剩余修复：4.5色阶 + 4.4/4.6小组合并 + 4.4唤醒率色阶 + 表头重上色 + 文档嵌入"""
from __future__ import annotations
import sys, io, json, subprocess, shutil, time
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")

SPREADSHEET_TOKEN = "M4NHsX8DHhcbTptLdFmcim4HnJh"
DOC_ID = "TBX2dTCIqoc3zJxHVdXcA1bUnoh"
LARK_CLI = shutil.which("lark-cli") or "lark-cli"


def col_letter(n):
    s = ""
    while n > 0:
        n, r = divmod(n - 1, 26)
        s = chr(ord("A") + r) + s
    return s


def run(cmd, stdin_text=None, timeout=120):
    r = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8",
                       shell=False, timeout=timeout, input=stdin_text)
    if r.returncode != 0:
        return False, r.stderr or r.stdout
    try:
        return True, json.loads(r.stdout)
    except:
        return False, r.stdout


def read_range(sid, rng):
    ok, resp = run([LARK_CLI, "sheets", "+read", "--spreadsheet-token",
                    SPREADSHEET_TOKEN, "--range", f"{sid}!{rng}"])
    if ok and resp.get("data"):
        return resp["data"].get("values", [])
    return []


def batch_set_style(data):
    dj = json.dumps(data, ensure_ascii=False)
    if len(dj) > 7500:
        half = len(data) // 2
        return batch_set_style(data[:half]) and batch_set_style(data[half:])
    ok, _ = run([LARK_CLI, "sheets", "+batch-set-style",
                 "--spreadsheet-token", SPREADSHEET_TOKEN, "--data", dj])
    return ok


def merge_cells(sid, rng):
    ok, _ = run([LARK_CLI, "sheets", "+merge-cells", "--spreadsheet-token",
                 SPREADSHEET_TOKEN, "--range", f"{sid}!{rng}", "--merge-type", "MERGE_ALL"])
    time.sleep(0.15)
    return ok


def lerp_color(ratio):
    if ratio >= 0.5:
        t = (ratio - 0.5) * 2
        r = int(255*(1-t)+76*t); g = int(215*(1-t)+175*t); b = int(0*(1-t)+80*t)
    else:
        t = ratio * 2
        r = int(244*(1-t)+255*t); g = int(67*(1-t)+215*t); b = int(54*(1-t)+0*t)
    return f"#{r:02X}{g:02X}{b:02X}"


def apply_color_to_col(sid, col_l, start_row, end_row):
    """读取一列数据并应用绿黄红色阶"""
    vals = read_range(sid, f"{col_l}{start_row}:{col_l}{end_row}")
    nums = []
    for row in vals:
        v = row[0] if row else None
        try:
            nums.append(float(str(v).replace("%", "").replace(",", "")))
        except:
            nums.append(None)
    valid = [n for n in nums if n is not None and n != 0]
    if not valid or max(valid) == min(valid):
        return 0
    mn, mx = min(valid), max(valid)
    ops = []
    for i, n in enumerate(nums):
        if n is None or n == 0:
            continue
        ratio = (n - mn) / (mx - mn)
        color = lerp_color(ratio)
        rng = f"{sid}!{col_l}{start_row + i}:{col_l}{start_row + i}"
        ops.append({"ranges": [rng], "style": {"backColor": color}})
    for i in range(0, len(ops), 30):
        batch_set_style(ops[i:i+30])
        time.sleep(0.2)
    return len(ops)


def group_merge(sid, col_l, start_row, end_row):
    """读取一列，相同值连续行做纵向合并"""
    vals = read_range(sid, f"{col_l}{start_row}:{col_l}{end_row}")
    cur_val = None; cur_start = None; merges = []
    for i, row in enumerate(vals):
        v = str(row[0]).strip() if row and row[0] else ""
        if v and v != cur_val:
            if cur_val and cur_start is not None and i - cur_start > 1:
                merges.append((cur_start + start_row, i + start_row - 1))
            cur_val = v; cur_start = i
    if cur_val and cur_start is not None and len(vals) - cur_start > 1:
        merges.append((cur_start + start_row, len(vals) + start_row - 1))
    for s, e in merges:
        merge_cells(sid, f"{col_l}{s}:{col_l}{e}")
    return len(merges)


def main():
    # === 4.5 色阶：外呼跟进率(D) + 综合有效跟进率(I) ===
    print("[1] 4.5 色阶...")
    sid_45 = "2T3bYQ"
    n1 = apply_color_to_col(sid_45, "D", 3, 106)
    n2 = apply_color_to_col(sid_45, "I", 3, 106)
    print(f"  D列: {n1}, I列: {n2}")

    # === 4.4 唤醒率色阶 ===
    print("[2] 4.4 唤醒率色阶...")
    sid_44 = "2SOCo8"
    # 删列后需要重新定位唤醒率列，读 row3 表头
    r3 = read_range(sid_44, "A3:BV3")
    if r3:
        header = r3[0] if r3 else []
        wake_cols = [i for i, v in enumerate(header) if v and "唤醒率" in str(v)]
        print(f"  唤醒率列(0-based): {wake_cols}")
        for ci in wake_cols:
            cl = col_letter(ci + 1)
            n = apply_color_to_col(sid_44, cl, 4, 99)
            print(f"    {cl}: {n} cells")

    # === 4.4 小组向下合并 ===
    print("[3] 4.4 小组合并...")
    n = group_merge(sid_44, "A", 4, 99)
    print(f"  {n} groups")

    # === 4.6 小组向下合并 ===
    print("[4] 4.6 小组合并...")
    n1 = group_merge("2TwScg", "A", 3, 98)
    n2 = group_merge("2TLJU4", "A", 3, 290)
    print(f"  系统外呼: {n1}, 企微回复比: {n2}")

    # === 重新上色表头+总计行 ===
    print("[5] 重新上色表头...")
    sheets_info = [
        ("2RK2qc", 98, 27, 2), ("2S16wg", 39, 12, 2),
        ("2SgAAo", 26, 29, 2), ("2SwpXO", 91, 27, 2),
        ("2SOCo8", 99, 74, 3), ("2T3bYQ", 106, 15, 2),
        ("2Ti90Y", 98, 9, 2), ("2TwScg", 98, 92, 2),
        ("2TLJU4", 290, 27, 2),
    ]
    for sid, n_rows, n_cols, n_hdr in sheets_info:
        ec = col_letter(n_cols)
        ops = [{"ranges": [f"{sid}!A1:{ec}{n_hdr}"],
                "style": {"font": {"bold": True, "font_size": 10},
                          "backColor": "#E1F0FE", "hAlign": 1, "vAlign": 1}}]
        batch_set_style(ops)
        time.sleep(0.3)
    print("  完成")

    # === 文档 overwrite ===
    print("[6] 文档 overwrite...")
    from pathlib import Path
    xml_path = Path(r"C:\Users\fengjianyi\Desktop\学情积分核算\_doc.xml")
    if xml_path.exists():
        xml = xml_path.read_text(encoding="utf-8")
    else:
        print("  XML 文件不存在，跳过")
        return
    ok, resp = run([LARK_CLI, "docs", "+update", "--api-version", "v2",
                    "--doc", DOC_ID, "--command", "overwrite",
                    "--doc-format", "xml", "--content", "-"],
                   stdin_text=xml, timeout=300)
    if ok:
        print(f"  result={resp.get('data',{}).get('result')}")
    else:
        print(f"  FAIL: {str(resp)[:300]}")

    print("\n[DONE]")
    print(f"文档：https://hcnig43mb8gp.feishu.cn/docx/{DOC_ID}")
    print(f"表格：https://hcnig43mb8gp.feishu.cn/sheets/{SPREADSHEET_TOKEN}")


if __name__ == "__main__":
    main()
