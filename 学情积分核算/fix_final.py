...
def batch_set_style(data):
    ...
    run([LARK_CLI, "sheets", "+batch-set-style",
...
def group_merge(sid, col_l, start_row, end_row):
    vals = read_range(sid, f"{col_l}{start_row}:{col_l}{end_row}")
    if not vals:
        print(f"    [WARN] no data for {sid}!{col_l}{start_row}:{col_l}{end_row}")
        return 0
...
def main():
    # 验证 +read 功能
    test = read_range("2T3bYQ", "D3:D5")
    print(f"[验证] +read 返回: {test}")
    if not test:
        print("ERROR: +read 不工作")
        return
    ...
    print(f"  外呼跟进率(D): {n1}, 综合有效跟进率(I): {n2}")

    # 2) 4.4 唤醒率色阶
    print("\n[2] 4.4 唤醒率色阶...")
    r3 = read_range("2SOCo8", "A3:CV3")
    if r3:
    ...
    for sid, n_rows, n_cols, n_hdr in sheets_info:
        ...
        batch_set_style([{"ranges": [f"{sid}!A1:{ec}{n_hdr}"],
