---
name: xueqing-credit-calc
description: 学情积分核算与豌豆币发放自动化。当用户提到"学情积分"、"学情课包核算"、"豌豆币发放"、"学情积分发放明细"、"OA 豌豆币申请"，或要求按 1号/16号 半月窗口结算海外思维学情课包预习+作业+消课的积分时调用。一条龙完成 BI 取数（续费规划表+上课明细）→ 4条件筛选（在课包池+预习=1+作业已提交+消课≥1）→ 积分计算（消课×500）→ 填发放模板 → Playwright 提交 OA。
when_to_use: 每月 1 号或 16 号需要核算上半月/下半月学情积分；用户给出区间要求生成"学情积分发放明细"；需要自动填写并提交 OA 豌豆币添加申请。
version: 3.1.0
---
...
1. **BI 取数** —— 下载续费规划表 + 上课明细到 `01_bi_exports/`
   - ⚠️ 如果下载超时 → 重试 3 次 → 失败则记录日志并终止
   - ⚠️ 如果文件大小 < 1KB → 视为无效，报错并终止
   - ⚠️ 如果遇到 `extendsion_loginlock_forbid` → 等 5-10 秒让会话释放后重试
2. **数据处理** —— 4条件筛选，输出 `03_output/<YYYYMMDD-YYYYMMDD学情积分发放明细>/积分汇总.xlsx` 等 3 个文件
   - ⚠️ 如果积分汇总为空（0行）→ 记录日志，跳过后续步骤
   - ⚠️ 如果总积分超出合理范围（>100000）→ 提示人工复核
   - ⚠️ 如果报 `PermissionError` → 关闭被 Excel 占用的输出文件后重跑
3. **生成发放模板** —— 用积分汇总的「获得积分明细」填模板，输出 `03_output/<期次>/发放豌豆币文档填写模板_YYYYMMDD.xlsx`（**文件名带当天日期，方便溯源**）
   - ⚠️ 如果模板文件被占用 → 等待10秒重试 → 失败则报错
   - ⚠️ 如果原始空模板 `发放豌豆币文档填写模板.xlsx` 不存在 → 终止并提示放置模板
4. **提交 OA**（仅 `--submit-oa` 时）—— Playwright 自动登录、点入口、填表、上传附件、提交
   - ⚠️ 如果 `auth_state.json` 失效 → 提示运行 `python scripts/oa_login/login_oa_qr.py` 扫码刷新
   - ⚠️ 如果字段验证失败（URL 还是 `method=add`）→ 看 `submit_step3_filled.png` 截图判断哪个字段空 → 重跑一次通常就过了
   - ⚠️ 如果提交后报 `TargetClosedError` → 通常意味着提交已成功，去 OA「我的申请」核实
5. **保留浏览器 60 秒** —— 便于人工核对
...
详见 [references/faq.md](references/faq.md)，覆盖以下场景：

- OA 提交失败 / 字段被误识别 / `TargetClosedError`
- 定时任务未执行
- BI 下载相关：`extendsion_loginlock_forbid`、0 bytes、`heap out of memory`、合并后空文件
- 数据处理 0 行 / `PermissionError`

## ❌ 危险操作黑名单

以下操作会导致核算失败或数据不准确，严禁执行：

1. ❌ **不要手动修改 `auth_state.json`**
   - 会导致登录态失效，OA 提交失败
   - 正确做法：运行 `python scripts/oa_login/login_oa_qr.py` 重新扫码

2. ❌ **不要在不同时间段复用报表文件**
   - BI报表数据实时更新，旧报表会导致积分核算不准确
   - 唯一例外：同一天内对同一时间段重复核算时可加 `--skip-fetch`
...
4. ❌ **不要跳过「数据是否无误」确认**
   - 除非是定时任务加 `--yes` 参数的无人值守模式
   - 人工确认可避免错误数据提交到 OA

5. ❌ **不要在 BI 导出未完成时关闭浏览器**
   - 会导致文件不完整或大小为 0
   - 等待「下载完成」提示后再关闭

6. ❌ **不要手动修改 `03_output/<期次>/积分汇总.xlsx`**
   - 模板填充会读取该文件，手动修改可能破坏数据结构
   - 如需调整，应修改核算规则后重新运行 `run --auto`

7. ❌ **不要 git commit `auth_state.json` 或 `record_events.json`**
   - 含会话 Cookie / 个人操作记录，属敏感信息
   - 已在 `.gitignore` 中，但手动 `git add -f` 时仍需警惕
