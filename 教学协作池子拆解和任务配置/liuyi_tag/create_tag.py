...
"""调六一工作台 userTag/addwithfile 接口创建两个标签：
   1. 「【海外】26年X月教学协作池-1-12课时低活」          ← dadou_ids_*.xlsx（豌豆大账号ID）
   2. 「【海外】26年X月教学协作池-1-12课时低活（益智）」 ← user_ids_*.xlsx（学员ID）
...
def main():
    ...
    ap.add_argument("--dadou-ids-xlsx", help="豌豆大账号 ID xlsx，默认 latest_inputs.json 里的")
    ...
    ap.add_argument("--suffix", default="", help="标签名追加后缀，例如 --suffix （模拟） 用于试跑")
    args = ap.parse_args()
    ...
    # 标签命名：【海外】26年X月教学协作池-{range}课时低活 / （益智）
    suffix = args.suffix or ""
    name_normal = f"【海外】{y[2:]}年{int(m)}月教学协作池-{course_range}课时低活{suffix}"
    name_yizhi = f"【海外】{y[2:]}年{int(m)}月教学协作池-{course_range}课时低活（益智）{suffix}"
    ...
    if not dadou_xlsx.exists():
        log(f"[ERROR] 找不到 dadou_ids xlsx: {dadou_xlsx}（先跑 filter.py）")
        sys.exit(1)
    if not user_xlsx.exists():
        log(f"[ERROR] 找不到 user_ids xlsx: {user_xlsx}")
        sys.exit(1)

    log(f"[INFO] 标签名:")
    log(f"  - 普通: {name_normal}  (file: {dadou_xlsx.name})")
    log(f"  - 益智: {name_yizhi} (file: {user_xlsx.name})")
    ...
    with sync_playwright() as p:
        ...
        for label, name, fp in (
            ("normal_tag", name_normal, dadou_xlsx),
            ("yizhi_tag",  name_yizhi,  user_xlsx),
        ):
            log("=" * 60)
