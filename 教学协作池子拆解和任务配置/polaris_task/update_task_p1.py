#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""P1服务池协作 - 北极星外呼平台：克隆既有「P1（学情反馈）」任务模板

工作流（与P0相同）：
  1. 复用 ../polaris_login/auth_state.json 进 sh-center.vipthink.cn
  2. POST /task/taskTemplate/16/list 搜任务（关键词「P1（学情反馈）」），按 createTime 取最新一条
  3. GET  /task/taskTemplate/getDetail?id={id} 拉详情
  4. 把 taskName 末尾的「-N月」替换为目标月份
  5. 读取 p1_group_ids_*.json 拿普通用户群的 groupId，替换详情里的 userGroupIds
  6. 替换 latestServiceTime 为用户传入的教师任务完成时间
...
def replace_month(name, target_month):
    """把任务名中「26年N月」的N替换为target_month
    例：【海外】26年5月教学协作任务-P1（学情反馈） → 【海外】26年6月教学协作任务-P1（学情反馈）
    """
    new, n = re.subn(r"(\d+年)(\d+)月", lambda m: f"{m.group(1)}{target_month}月", name)
    if n == 0:
        # 兜底：如果没匹配到「N年N月」，尝试匹配末尾的「-N月」
...
def main():
    ...
    ap.add_argument("--group-ids-json", help="p1_group_ids_*.json，默认 output/p1_group_ids_{today}.json")
    ap.add_argument("--source-task-id", type=int, help="基线任务 id；不传则按 keyword 搜最新一条")
    ap.add_argument("--suffix", default="", help="新任务名追加后缀")
    ...
    group_ids_path = Path(args.group_ids_json) if args.group_ids_json else OUTPUT_DIR / f"p1_group_ids_{today_tag}.json"
    if not group_ids_path.exists():
        log(f"[ERROR] 找不到 {group_ids_path}（先跑 liuyi_tag/create_group_p1.py）")
        sys.exit(1)
    ...
    with sync_playwright() as p:
        ...
        log(f"[STEP 5] payload 摘要: businessId={payload.get('businessId')!r}, "
            ...
            f"latestServiceTime={payload.get('latestServiceTime')!r}")
        ...
        out = OUTPUT_DIR / f"p1_polaris_task_{today_tag}.json"
